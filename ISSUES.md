...
| 34 | **17 个孤儿组件**（零 import），其中 `vps-dashboard/monitor-{cards,detail,table}.jsx` + `tab-monitor.jsx` 合计 **约 31.7KB 死 UI**；`ai-chat-entry.jsx` / `ai-float-window.jsx` 同样零引用 | 各文件 | **待祥哥定夺**（2026-09-13 复核：数字没错，**17 个**（`entry/basic.js` 是误报 —— 它被 `vite-build.js` 生成 HTML 时按 `/js/basic-<版本>.js` 引用，不是模块 import）。但「修」在这里不等于「删」，按 git 溯源分两类：<br>**① 上游继承的死代码 6 个** —— `terminal/build-ls-term-id.js`(2022-07)、`tabs/on-tab-drop.js`(2024-12)、`common/component.jsx`(2025-02)、`setting-panel/on-tree-drop.js`(2025-02)、`terminal/event-emitter.js`(2026-02)、`terminal-info/log-path-edit.jsx`(2026-05)。按 #51 同一口径**不动**。<br>**② 自研但从未接线的功能 11 个** —— `terminal-info/route-detect.jsx`(**25KB**)、`vps-dashboard/{monitor-cards,monitor-detail,monitor-table,tab-monitor}.jsx`(**约 31.7KB**)、`setting-panel/tab-nezha.jsx`(9.3KB)、`side-panel-r/side-panel-r.jsx`、`deploy/deploy-agent.js`、`ai/ai-float-window.jsx`、`ai/ai-chat-entry.jsx`、`terminal-info/terminal-info-entry.jsx`。<br>⚠️ **其中 `tab-nezha.jsx`、`ai-float-window.jsx` 在 2026-09-11 那次「界面与日志全面汉化」里还被改过** —— 说明这些不是陈年垃圾而是**没接完线的功能**，一键删掉 = 删掉祥哥自己最近的活，所以**不擅自删**。要定的是：**接线启用**，还是确认废弃后删） |
