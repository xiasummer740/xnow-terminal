...
| 17 | 无 `playwright.config.js` → Playwright 默认扫描整个仓库，把 `test/unit/*` 全部误收集 | 项目根 | 已修已验（2026-09-13：补 `playwright.config.js` 钉 `testDir: './test/e2e'`。实测 `npx playwright test --list` → **56 tests in 48 files，`test/unit/` 收录 0 个**） |
...
| 20 | Agent 循环**中止不了**且结果无上限膨胀：`createAIClient` 无 timeout、`stream:false` 从不注册 session 故 `stopStream` 无 sessionId 可杀；工具结果原样 push 进 messages（单次可达 1-2MB × 150 轮） | `lib/ai.js:15-30,51-120`、`ai/agent.js:133-158,320-457` | 已修已验（2026-09-13，两半一起收）：<br>**① 停不下来** —— 根因是 `createAIClient` 无 timeout，且 `AIchatWithTools` 走 `stream:false` **从不注册 session**，所以 `stopStream` 根本没有 sessionId 可杀；界面上点「停止」只是把 `abortRef.current` 置 true，得等这一次 HTTP 自己回来、跑到下一轮循环开头才会被看见 —— 模型/网关卡住时等于没反应。改法：按 `runId` 在主进程登记 `AbortController`，新增 `abortAIChat` IPC，点停止直接掐掉在飞的请求；另补 **3 分钟超时**兜底。**实测**：起一个收到请求就挂着、永不回包的本地服务，`AIchatWithTools` 卡住后调 `abortAIChat` —— 请求 **518ms** 内返回 `{aborted:true}`（其中 500ms 是脚本自带延时）。<br>**② 结果膨胀** —— 工具结果原来原样 push 进 messages，单条可上兆；改为进上下文前截断到 2 万字（留头尾、中间省略），这也是「越用越卡」的直接来源。<br>⚠️ **待祥哥真机复核**：AI 面板里点「停止」的实际手感 |
...
| 34 | **17 个孤儿组件**（零 import），其中 `vps-dashboard/monitor-{cards,detail,table}.jsx` + `tab-monitor.jsx` 合计 **约 31.7KB 死 UI**；`ai-chat-entry.jsx` / `ai-float-window.jsx` 同样零引用 | 各文件 | **不修**（祥哥 2026-09-13 拍板：**全部保留，一个都不删**。已给这 17 个文件逐个加头注释标注「零引用 · 有意保留」，按来源分三组措辞 —— 上游继承 6 / 重构残留 2 / 自研未接线 9；**纯注释新增、0 行逻辑改动**，`standard` 通过）。复核依据：数字没错，**17 个**（`entry/basic.js` 是误报 —— 它被 `vite-build.js` 生成 HTML 时按 `/js/basic-<版本>.js` 引用，不是模块 import）。但「修」在这里不等于「删」，按 git 溯源分两类：<br>**① 上游继承的死代码 6 个** —— `terminal/build-ls-term-id.js`(2022-07)、`tabs/on-tab-drop.js`(2024-12)、`common/component.jsx`(2025-02)、`setting-panel/on-tree-drop.js`(2025-02)、`terminal/event-emitter.js`(2026-02)、`terminal-info/log-path-edit.jsx`(2026-05)。按 #51 同一口径**不动**。<br>**② 自研但从未接线的功能 11 个** —— `terminal-info/route-detect.jsx`(**25KB**)、`vps-dashboard/{monitor-cards,monitor-detail,monitor-table,tab-monitor}.jsx`(**约 31.7KB**)、`setting-panel/tab-nezha.jsx`(9.3KB)、`side-panel-r/side-panel-r.jsx`、`deploy/deploy-agent.js`、`ai/ai-float-window.jsx`、`ai/ai-chat-entry.jsx`、`terminal-info/terminal-info-entry.jsx`。<br>⚠️ **其中 `tab-nezha.jsx`、`ai-float-window.jsx` 在 2026-09-11 那次「界面与日志全面汉化」里还被改过** —— 说明这些不是陈年垃圾而是**没接完线的功能**，一键删掉 = 删掉祥哥自己最近的活，所以**不擅自删**。**定论（2026-09-13）：全部保留，一个都不删**。 |
...
| 41 | 迁移版本账本**丢失后不重跑迁移，而是全部静默跳过**（`shouldUpgrade()` 的 `dbVersion === emptyVersion` 兜底直接 return false）；账本本身存在 sqlite 的 `data` 表，不在 `.nedb` | `migrate/index.js:52-74` | 已修（2026-09-13，**原描述要纠正，这就是那句丢了的「见下」**）：<br>**真实安装/升级根本不走这个文件** —— `ipc.js` 要的是 `upgrade/index.js`，那条路在 `dbVersion === emptyVersion` 时会 `initData()` **并盖上版本号**，不会静默跳过。会静默跳过的只有 `migrate/index.js`，而它只被 `migrate-1-to-2` 调用一次（nedb→sqlite 那次迁移）。<br>**实测祥哥本机**：安装版库 `xnow_data.db` 里 `version` 行是 `3.17.11`、账本完好 —— 所以这条**当前没在他身上发作**，是潜伏缺陷，不是正在丢数据的 bug。<br>改法只补那一处：账本为空时不再哑掉，写一条 `log.warn` + 把版本号盖上去；**不动「跳过历史迁移脚本」这个决定**（那些脚本是给上古格式写的，`v1.3.0` 甚至要读 `electerm-user-config.json`；从 nedb 搬过来的数据已经是新格式，重放反而危险）。<br>**实测**：桩掉 `dbAction` 与 `app-props` 跑 `checkDbUpgrade()`，确认 `data.update`（盖版本号）这次真的发生了 —— 改前只有 `data.findOne` ×2，改后多出 `data.update` + `dbUpgradeLog.insert` |
...
| 46 | 凭据加密遗留路径：**固定全零 IV 的 CBC**、`isLegacyFormat` 把纯十六进制明文误判为密文、解密失败原样返回 | `lib/enc.js:15-16,71-72,99-112` | 不修（祥哥 2026-09-12 拍板：单机自用，不存在多租户/多人共享库的场景，遗留密文形态的收益低于改动风险）<br>⚠️ 残留风险留档：**整库明文落盘**是 #21，跟本条同源，那条要另算 |
...
| 48 | 自定义 CSS 走 `style.innerHTML`，只过滤了 `@import` | `bg/custom-css.jsx:16-17` 等 | 已修已验（2026-09-13：抽出 `client/common/sanitize-css.js`，url() 只放行 `data:`/`blob:`/无 scheme 的相对写法，其余（http(s)/file/ftp/自定义协议）替换为 `none`。**"拦 url() 会误伤正常背景图"这个顾虑实测不成立**：主窗口 CSP 写着 `img-src 'self' data: blob:` / `font-src 'self' data:`，远程 url 本来就被 CSP 挡着加载不出来。单测 `test/unit-ci/sanitize-css.spec.js` **20/20**，含 7 条放行用例防误伤 + 1 条恒等函数反证） |
...
| 51 | 3 个继承自上游的死代码文件零引用：`download-mirrors.js`、`get-category-color.js`、`key-shift-pressed.js` | `client/common/` | 不修（祥哥 2026-09-12 拍板：上游继承的死代码，零引用不影响功能；删了反而让后续合并上游时多出无谓冲突 —— 按项目规则「预存死代码不动」） |
...
| 65 | **开发模式"独立数据目录"从来没生效过**：`create-app.js` 里给 `process.env.DATA_PATH` 赋值的时机，晚于数据库解析路径（`db.js` 在模块顶层就 `createDb()` 了）；而且**没有任何模块会读它**（读 DATA_PATH 的只有 nedb/sqlite/storage-key，全在那之前加载）。实际后果：`npm start` 和安装版**共用** `%APPDATA%/xnow-terminal` —— dev 下的改动、测试直接落在真实数据上 | `lib/create-app.js`（已删）、`lib/dev-data-path.js`（新增）、`app.js:24-27` | 已修已验（2026-09-13 祥哥拍板：**首次把安装版数据复制过去**）：<br>原来那句赋值在 `create-app.js` 里、晚于 `db.js` 的模块顶层 `createDb()`，对数据库完全无效，唯一作用是打印一行根本不存在的目录来误导人。新增 `lib/dev-data-path.js`，改到 `app.js` 顶部、`require('./lib/create-app')` **之前**调用 —— 和 #40 的 clear-config 同一位置、同一理由。dev 目录不存在且安装版有数据时，先把 `users/`（约 1M 真数据）复制过去；`Cache`/`GPUCache`/`logs` 不复制（那是 Electron 自己的 userData 垃圾，白占十几兆）。之后每次启动都走自己的数据，不会再被覆盖。复制失败只记错误、不挡启动。<br>**实测**：开发版启动后 `%APPDATA%/xnow-terminal-dev/users/default_user/xnow.db` 出现，与安装版逐项比对一致（terminalThemes 2 条、version 均 `3.17.11`）；**安装版目录 mtime 仍是 9月12日 11:34，一个字节没动**；启动日志两行齐全、无报错。<br>⚠️ **待祥哥真机复核**：`npm start` 打开后书签/主题/SSH 主机是否都在 |
...
**补充：2026-09-13 19:55 哨兵报 `bookmarks 5 → 4`，排查后确认为用户手动删除，非故障。**

判据（三条都指向"单条删除"，与"同步覆盖"的特征相反）：

| 观察 | 单条删除 | 同步覆盖会怎样 |
|---|---|---|
| 减少条数 | 只少 1 条 | 全表被覆盖 |
| `bookmarkGroups.default.bookmarkIds` | **未变**，仍是 5 个 id | 必然跟着一起改 |
| `bg-ping.json` 里该 IP | **仍在** | — |

被删的是 `VBrvWnf`（JP.TKY.BGP / 38.47.109.110），经用户确认是他本人在界面上删的。

**教训**：单条增删与故障在小样本下**无法区分**——哨兵每有变动就报警 = 狼来了，会被真实故障淹没。已改为只在「一次少 2 条以上 / 归零 / 读库出错」时告警，单条变动静默。
> 与 #7 的关系：这次**不是** #7。不要因为"又少了一条"就回头怀疑同步防护失效——先看分组引用有没有被动过，那是最快的判别点。

### #68 AI 用 `open_tab` 开的标签没有密码 → 每次连接都弹密码框（用户可见，待修）

**现象**：祥哥报告「跟 AI 对话几次后，它就弹出让我输密码」，具体是 YX HK（`root@87.83.104.229:22`），且是「老是」要输。

**先排除数据损坏**（这一步做了，结论是数据没问题）：

- 库里该条密码完好：14 位，数字 4 / 小写 4 / 大写 4 / 符号 2，**无空白、无不可见字符、无非 ASCII、无引号反斜杠**
- 与 `xnow-2026-09-11.db` 备份**逐字节一致** → 本次恢复没有破坏它
- 同库另外两条存密码的记录（HY US 12 位、RN US 16 位）同样干净

**根因：AI agent 有两条开标签的路，只有一条带凭据。**

| 工具 | 路径 | 凭据 |
|---|---|---|
| `open_bookmark`（按 id） | `mcp-handler.js:264-277` → `store.onSelectBookmark(id)` → `setting.js:63-81` 把**完整 bookmark 对象**（含 `password`/`privateKey`/`passphrase`/`authType`…）展开进 `addTab` | **有** ✅ |
| `open_tab`（按连接参数） | `mcp-handler.js:471-495` 只用 `fixBookmarkData({ ...args })`，**不查任何书签** | **没有** ❌ |

而模型**根本拿不到真实凭据**——这是故意的安全设计：`sanitizeBookmark`（`mcp-handler.js:183-201`）在 `list_bookmarks` / `get_bookmark` 返回前剥掉
`password`、`privateKey`、`passphrase`、`certificate`、`proxy`、`connectionHoppings`、`sshTunnels`。

`fixBookmarkData`（`fix-bookmark-default.js:59-89`）**只补默认值**（`authType: 'password'`、端口等），不会凭空注入任何真实凭据。

⇒ 合起来：AI 走 `open_tab` 开的标签，带着「要密码」的默认设置、**却没有任何密码** → SSH 必然弹框。
⇒ 而且每次都是一张新标签，**输进去的密码不会被写回书签** → 所以「老是」要输。

**结论**：与本次数据丢失无关，与恢复也无关，是本来就存在的设计缺口。数据没坏，是 AI 开标签这条路缺凭据。

**修复方向（待拍板）**：`mcpOpenTab` 在构造 tab 前，按 `host + port + username` 匹配已有书签，命中则把凭据合并进 tab。
...
### #70 发版门禁在本项目**从未生效**：正则只认第 1 列、且不认括号补充（2026-09-13 实测）

**现象**：`xnow release` 报「✅ 发版门禁第1问：ISSUES.md 已清零」，但 ISSUES.md 里**实际有 7 项「待修」**在表格中（#11 #18 #35 #37 #64 #66 #67）。

**根因**：闸门正则在 `xiangge-env/dotfiles/xnow-wrapper.sh:136`：

```bash
pending=$(grep -cE '^\|\s*(待修|已修待验)\s*\|' "$root/ISSUES.md")
```

它要求**状态列是第 1 列**、且单元格内容**恰好只有**「待修」二字。而：

| | 状态列位置 | 实际写法 |
|---|---|---|
| 官方模板 `dotfiles/ISSUES.template.md` | 第 **1** 列 | `\| 待修 \| （示例）… \|` ✅ 能匹配 |
| 本项目 `ISSUES.md` | 最后 1 列 | `\| 11 \| … \| … \| 待修（2026-09-13 复核：…） \|` ❌ 匹配不到 |

**两条都不满足** → `grep -c` = **0** → 报「已清零」。

**实测对照**（同一条 ISSUES.md）：

```
闸门的正则            ^\|\s*(待修|已修待验)\s*\|      → 匹配 0 处
实际情况              \|\s*(待修|已修待验)             → 匹配 7 处（行 32/46/70/72/89/91/92）
```

**影响**：本项目**每一次发版都是在"闸门假绿"下进行的**，包括 v3.17.12。门禁形同虚设，起不到「有未修问题就拦住」的作用。

**旁证**：`xnow-wrapper.sh:134` 的注释写着「只匹配表格状态列，不全文子串匹配——模板说明文字含「待修/已修待验」裸词会假阳性」。作者是为了**避免假阳性**（模板第 7 行说明文字）才收窄了正则，**代价是引入了假阴性**——只要状态列带任何括号补充（而本项目的写法**每一条都带**）就全部漏掉。

**修法（待祥哥拍板，属 xiangge-env 侧）**：改成**列序无关**、允许状态单元格后接补充说明：

```bash
grep -cE '^\|.*\|\s*(待修|已修待验)[^|]*\|' "$root/ISSUES.md"
```
