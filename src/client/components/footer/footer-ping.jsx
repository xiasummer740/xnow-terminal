...
function SignalIcon ({ level }) {
  const bars = [0, 1, 2, 3].map(i => {
    ...
    return (
      <rect
        key={i} x={i * 4.5 + 1} y={y} width='3.5' height={h} rx='0.5'
...
export default function FooterPing ({ store }) {
  ...
  useEffect(() => {
    ...
    const measure = async () => {
      ...
      try {
        ...
        if (latency > 0) {
          setPing(latency + 'ms')
          if (latency < 60) { setColor('#52c41a'); setLevel(3) } else if (latency < 120) { setColor('#52c41a'); setLevel(2) } else if (latency < 200) { setColor('#faad14'); setLevel(1) } else { setColor('#ff4d4f'); setLevel(0) }
        }
      } catch (e) {}
    }
    measure()
  ...
  return (
    <div
      className='terminal-footer-unit terminal-footer-ping'
      style={{
        color,
        fontSize: 12,
        fontWeight: 'bold',
