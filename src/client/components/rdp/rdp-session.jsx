...
export default class RdpSession extends PureComponent {
  ...
  handleUploadButtonClick = async () => {
    ...
    const files = await window.api.openDialog({
      title: window.translate('Choose files to upload to remote desktop'),
      message: window.translate('Choose files to upload'),
      properties
    }).catch((err) => {
    ...
    return (
      ...
      >
        <div className='fleft'>
          ...
          <HelpIcon
            link='https://github.com/xiasummer740/xnow-terminal/wiki/RDP-File-Transfer'
            className='mg2r mg1l'
          />
        </div>
